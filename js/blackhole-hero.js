...
`;
  ...
  function readConfig(host) {
    ...
    return {
      distance: num(d.distance, 24),
      elevation: num(d.elevation, -5.5),
      azimuth: num(d.azimuth, 0),
      orbitSpeed: num(d.orbitSpeed, 0),
      roll: num(d.roll, -20),
      fov: num(d.fov, 42),
      diskInner: num(d.diskInner, 3),
      diskOuter: num(d.diskOuter, 15),
      diskThickness: num(d.diskThickness, 0.26),
      diskDensity: num(d.diskDensity, 1),
      brightness: num(d.brightness, 1),
      spinSpeed: num(d.spinSpeed, 0.06),
      grain: num(d.grain, 0.48),
      doppler: num(d.doppler, 0.35),
      hotColor: d.hotColor || "#FFF3DE",
      midColor: d.midColor || "#FF9838",
      coolColor: d.coolColor || "#8E3A0B",
      starBrightness: num(d.starBrightness, 0),
      glow: num(d.glow, 1),
      exposure: num(d.exposure, 0.9),
      vignette: num(d.vignette, 0.28),
      steps: num(d.steps, 300),
      resolution: num(d.resolution, 0.7),
      maxDpr: num(d.maxDpr, 1.75),
      focus: parseFocus(d.focus),
      scrim: d.scrim || "left",
      scrimStrength: num(d.scrimStrength, 0.9),
  ...
  function settlePasses() {
    return prefersReducedMotion() || isMobile() ? 16 : 1;
  }
  ...
  function mountBlackHoleHero(host) {
    ...
    const baseConfig = readConfig(host);
    const props = { current: effectiveConfig(baseConfig) };
    ...
    function resize() {
      props.current = effectiveConfig(baseConfig);
      ...
      const scale = software
        ? 0.34
        : Math.min(1, Math.max(0.4, C.resolution));
    ...
    function render(t) {
      ...
      const RY = ry * cr + uy * sr;
      const RZ = rz * cr + uz * sr;
      const UX = -rx * sr + ux * cr;
      const UY = -ry * sr + uy * cr;
      const UZ = -rz * sr + uz * cr;
    ...
    const ro = new ResizeObserver(() => {
      resize();
      if (reduced || props.current.paused || isMobile()) settle(16);
    });
    ro.observe(host);
  ...
  function init(root) {
    ...
    const hosts = scope.querySelectorAll("[data-blackhole-hero]");
    return Array.from(hosts).map((host) => mountBlackHoleHero(host));
  }

  window.initBlackHoleHero = init;
